...
shape1 = (size1, size1, 3)				# shape for the non padded images
shape2 = (size2, size2, 3)				# shape for the padded images
...
if baseline == "MobileUNet":
    ...
    csv_name = "./Results/training_FM_MU.csv"
    csv_name2 = "./Results/validation_FM_MU.csv"

    # Path for saving Weights
    weights_folder1= "./Weights/weights_MU"
    weights_folder= "./Weights/weights_FM_MU"
    ...
    # CSV files for training and validation    
    csv_name = "./Results/training_FM_RU.csv"
    csv_name2 = "./Results/validation_FM_RU.csv"

    # Path for saving Weights
    weights_folder1= "./Weights/weights_RU"
    weights_folder= "./Weights/weights_FM_RU"
...
def base_model():
    ...
    output1 = model1(inp1)
    transforms_w,inv_transforms_w = weak_transforms(batch_size1)  	 # function is defined in augment.py, weak augmentation parameters
    transforms_s,inv_transforms_s = strong_transforms(batch_size1) 	 # function is defined in augment.py, strong augmentation parameters
    #output1 = Cropping2D(cropping=padding_pixels,name='labelled')(output1)	# comment this line if padding is not used
    output2 = AugmLayer(transforms_s, output_dim=None, preproc_input=None,name='aug')(inp2, training=True)   
    output2 = model1(output2)      
    output2 = AugmLayer(inv_transforms_s, output_dim=None, preproc_input=None,name='rev_aug')(output2, training=True)
    #output2 = Cropping2D(cropping=padding_pixels,name='unlabelled_w')(output2)   # comment this line if padding is not used
    model = Model(inputs=[inp1,inp2],outputs=[output1,output2], name='fixmatch_model')
    return model
...
def apply_gradient(optimizer, model, x_l , x_u, y_l):
    transforms_w,inv_transforms_w = weak_transforms(batch_size1)  	 
    transforms_s,inv_transforms_s = strong_transforms(batch_size1) 	 
    model.get_layer('aug').transforms = transforms_w
    model.get_layer('rev_aug').transforms = inv_transforms_w    
    y_dummy, y_pred_w = model([x_l,x_u])
    model.get_layer('aug').transforms = transforms_s
    model.get_layer('rev_aug').transforms = inv_transforms_s 
    with tf.GradientTape() as tape:
        y_pred_l, y_pred_s = model([x_l,x_u])
        loss_l = custom_ce(y_l, y_pred_l)
        loss_u = loss_ul(y_pred_w, y_pred_s)
...
for lambda_u in lambda_set:
    model.save_weights('fixmatch_train.h5')     # For making the same random initiliazation for all the lambda values, initial weight is stored
    final_epoch=20
    ...
    for j in range(epoch1,final_epoch):
        ...
        with open(csv_name2, 'a+', newline='') as f:
             f1 = csv.writer(f)
             f1.writerow(history)    
    model.load_weights('fixmatch_train.h5')    # For making the same random initiliazation for all the lambda values, initial weight is loaded
    epoch1=0         
